...
```sh
docker pull ledokku/ledokku:0.0.3
docker tag ledokku/ledokku:0.0.3 dokku/ledokku:0.0.3
```

Finally we deploy the tag to start our ledokku server.

```sh
dokku tags:deploy ledokku 0.0.3
```
...
In the between please take a look at the dokku official documentation http://dokku.viewdocs.io/dokku/deployment/application-deployment/.

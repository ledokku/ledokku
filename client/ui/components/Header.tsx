...
export const Header = () => {
  ...
  return (
    <Navbar variant="floating" css={{ background: "transparent" }} suppressHydrationWarning={true}>
      ...
      <Navbar.Content variant='underline-rounded' hideIn="xs">
        {menuItems.map((it, index) => {
          const id = useId();
          return <Navbar.Link id={id} key={index} isActive={location.pathname === it.link} href={it.link}>{it.name}</Navbar.Link>;
        })}
      </Navbar.Content>
      <Navbar.Collapse>
        {menuItems.map((it, index) => <Navbar.CollapseItem key={index} isActive={location.pathname === it.link}><Link color="inherit" href={it.link}><span>{it.name}</span></Link></Navbar.CollapseItem>)}
      </Navbar.Collapse>
      <Navbar.Content>
        <Navbar.Item>
          <Dropdown>

...
export const App = () => {
  ...
  if (appByIdLoading || databaseDataLoading) {
    // TODO nice loading
    return <p>Loading...</p>;
  }
  ...
  const linkedDatabases = appById?.app?.databases;
  ...
  return (
    <div>
      ...
      {appByIdError ? (
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <Alert status="error">
            ...
            <AlertDescription>{appByIdError.message}</AlertDescription>
          </Alert>
        </div>
      ) : null}

      {!appByIdLoading && !appByIdError && !appById?.app ? (
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <Alert status="error">
      ...
      {appById?.app ? (
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <TabNav>
            <TabNavLink to={`/app/${appById.app.id}`} selected>
              App
            </TabNavLink>
            <TabNavLink to={`/app/${appById.app.id}/logs`}>Logs</TabNavLink>
            <TabNavLink to={`/app/${appById.app.id}/env`}>Env setup</TabNavLink>
            <TabNavLink to={`/app/${appById.app.id}/settings`}>
              Settings
            </TabNavLink>
          </TabNav>
        </div>
                  ...
                  {notLinkedDatabases.length !== 0 ? (
                    ...
                    <React.Fragment>
                      <p className="mt-3 mb-3 text-cool-gray-400">
                        All your databases are already linked to this app! If
                        you want to create more databases proceed with create
                        database flow.
                      </p>
                      <div className="ml-80">
                        <Link to="/create-database">
